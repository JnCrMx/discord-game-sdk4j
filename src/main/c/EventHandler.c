...
void on_lobby_message(void* event_data, int64_t lobby_id, int64_t user_id, uint8_t* data, uint32_t data_length)
{
	...
	jclass clazz = (*env)->GetObjectClass(env, event_struct->handler);
	jmethodID method = (*env)->GetMethodID(env, clazz, "onLobbyMessage", "(JJ[B)V");
...
void on_message(void* event_data, DiscordNetworkPeerId peer_id, DiscordNetworkChannelId channel_id, uint8_t* data, uint32_t data_length)
{
	...
	jclass clazz = (*env)->GetObjectClass(env, event_struct->handler);
	jmethodID method = (*env)->GetMethodID(env, clazz, "onMessage", "(JB[B)V");

	jbyteArray array = (*env)->NewByteArray(env, data_length);
	(*env)->SetByteArrayRegion(env, array, 0, data_length, data);

	(*env)->CallVoidMethod(env, event_struct->handler, method, peer_id, channel_id, array);
}
...
void on_route_update(void* event_data, const char* route_data)
{
	...
	jstring route_data_string = (*env)->NewStringUTF(env, route_data);

	jclass clazz = (*env)->GetObjectClass(env, event_struct->handler);
	jmethodID method = (*env)->GetMethodID(env, clazz, "onRouteUpdate", "(Ljava/lang/String;)V");
	(*env)->CallVoidMethod(env, event_struct->handler, method, route_data_string);
